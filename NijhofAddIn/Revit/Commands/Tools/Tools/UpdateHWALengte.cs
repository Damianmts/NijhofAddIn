...
namespace NijhofAddIn.Revit.Commands.Tools.Tools
{
    ...
    internal class UpdateHWALengte : IExternalCommand
    {
            ...
            ElementSet elements)
        {
            ...
            using (Transaction tx = new Transaction(doc, "Update Pipe Length"))
            {
                ...
                foreach (Pipe pipe in collector)
                {
                    ...
                    if (abbreviationParam != null && (abbreviationParam.AsString() == "M521" || abbreviationParam.AsString() == "M5210"))
                    {
        ...
        private Connector GetConnectedFittingConnector(Connector pipeConnector)
        {
            ...
            {
#if RELEASE2023
                if (refConnector.Owner is FamilyInstance && refConnector.Owner.Category.Id.IntegerValue == (int)BuiltInCategory.OST_PipeFitting)
#elif RELEASE2024 || RELEASE2025
                if (refConnector.Owner is FamilyInstance && refConnector.Owner.Category.Id.Value == (int)BuiltInCategory.OST_PipeFitting)
#endif
                {
                    return refConnector;
                }
        ...
        private void MoveAndReconnectElements(Document doc, Connector pipeConnector, Connector fittingConnector, double newZ)
        {
